...
describe('Recipes Endpoints', ()=> {
  ...
  const { testUsers, testIngredients, testIngredientType, testRecipes, testFavorites } = helpers.makeFixtures()
  ...
  describe('GET /recipes', () => {
    ...
    it('Responds with 200 and all the Recipes', () => {
      const expectedRecipes = testRecipes.map(recipe => helpers.makeExpectedRecipes(testIngredientType, recipe))
      return supertest(app)
        .get('/recipes')
    ...
    it('Responds with 200 and Recipe when filter by ingredient', ()=>{
      const expectedIngredientRecipe = testRecipes.map(recipe => helpers.makeExpectedRecipes(testIngredientType, recipe))
      const queryIngredient = testIngredient.ingredient_cat

      return supertest(app)
        .get('/recipes')
        .query({keyword: '', filter: queryIngredient})
        .set('Authorization', helpers.makeAuthHeader(testUser))
        .expect(200, [expectedIngredientRecipe[1]])
    })
    it('Responds with 200 and Recipe when keyword filter by name', ()=>{
      ...
      return supertest(app)
        ...
        .set('Authorization', helpers.makeAuthHeader(testUser))
        .expect(200, [expectedIngredientRecipe[0]])
    })
    it('Responds with 200 and Recipe when keyword and filter', ()=>{
      ...
      return supertest(app)
        .get('/recipes')
        .query({keyword: queryKeyword, filter: queryIngredient})
        .set('Authorization', helpers.makeAuthHeader(testUser))
        .expect(200, [expectedIngredientRecipe[1]])
    })
    })
  ...
  describe('GET /recipes/:id', () => {
    ...
    context('Given there are recipes', () => {
      ...
      it('Responds 200 with recipe', () => {
        ...
        //testRecipes.map(recipe => helpers.makeExpectedRecipes(testIngredientType, recipe))
